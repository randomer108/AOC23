...
class hand:
    ...
    def count_cards_2(self):
        ...
        #print(f'{self.hand_counted2} and {wild}')
        
        cards = self.hand_counted2 # bt more terse

        if   self.hand_raw =='JJJJJ':                        self.type2='5ofaK' 
        ...
        elif max(cards.values()) ==2 and len(cards)==3:      self.type2='2pair' # with 1 J  - 2 pair is dominated by 3ofaK. With >2 J, 2 pair is obviously also dominated.

        elif max(cards.values()) ==2 and len(cards)==4:      self.type2='1pair'
        elif max(cards.values())+wild ==2 and len(cards)==4: self.type2='1pair'  

        elif max(cards.values()) ==1   and wild==0:          self.type2='highCard' # added condition of no wilds. If you have wilds, you should not get here! So this is wrong!
        else: raise Exception('Hand type not recognised')
...
# Recount the same hands
Hands=[hand.count_cards_2() for hand in Hands]


# new sorting (Jacks at end)
